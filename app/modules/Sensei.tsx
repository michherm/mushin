...
/**
 * SENSEI — der KI-Coach als State Intervention Architecture.
 *
 * Diese Version nutzt die API-Route /api/sensei.
 * Claude läuft im Backend, der API-Key bleibt dort.
 * Bei Fehler: lokale Engine als Fallback (Nutzer merkt nichts).
 */
...
export function Sensei({
  ...
  /* Auto-Eskalation: drei Spiralen in Folge → Reduktion erzwingen */
  useEffect(() => {
  ...
  if (loading) {
    return (
      <Stage>
        ...
        <div className="h-8" />
        <div className="text-ink-dim italic font-serif text-[20px] animate-pulse">
          ...
        </div>
      </Stage>
    );
  ...
  if (!response) {
    return (
      <Stage>
        ...
        <p className="text-ink-dim text-[16px] mb-7">
        ...
        <textarea
          ...
          placeholder="…"
          className="w-full min-h-[100px] bg-bg-raise text-ink border border-line focus:border-line-hi outline-none px-5 py-4 font-serif text-[20px] font-light leading-[1.4] resize-none"
        />
        <div className="h-6" />
  ...
  return (
    <Stage>
      <Tag>Antwort</Tag>
      <div className="h-7" />
      <Display>{intervention.line1}</Display>
      {intervention.line2 && (
        <>
          <div className="h-5" />
          <p
            className="text-ink-dim leading-[1.5] max-w-[520px] mx-auto"
      ...
      {intervention.line3 && (
        <>
          <div className="h-3" />
          <p
            className="leading-[1.5] max-w-[520px] mx-auto"
            style={{ fontSize: 'clamp(16px, 2vw, 19px)', color: '#6a6458' }}
          >
            {intervention.line3}
          </p>
        </>
      ...
      <div className="flex gap-3 flex-wrap justify-center">
        ...
        {intervention.action === 'breath'    && <Btn onClick={() => onModule('breath')}>Zur Atmung</Btn>}
        {intervention.action === 'emergency' && <Btn kind="ember" onClick={() => onModule('emergency')}>Reduktion</Btn>}
        {intervention.action === 'flow'      && <Btn onClick={() => onModule('flow')}>Beobachten</Btn>}
